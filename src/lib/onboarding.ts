...
export const ONBOARDING_MILESTONES: OnboardingMilestone[] = [
  ...
  },
  {
    ...
    description:
      "Set your Campaign as active to start building Characters and adventures.",
    targetPages: ["/campaigns"],
  ...
  },
  {
    ...
    description:
      "Factions group Characters by allegiance. Start with the heroes.",
...
export function getCurrentMilestone(
  ...
  return (
    ONBOARDING_MILESTONES.find(milestone => {
 ...
 */
export function getMilestonesForPage(
  currentPath: string
): OnboardingMilestone[] {
  return ONBOARDING_MILESTONES.filter(milestone =>
    milestone.targetPages.some(
      page =>
        page === currentPath ||
        currentPath.match(new RegExp(page.replace(/\[.*?\]/g, "[^/]+")))
    )
  )
}
 ...
 */
export function isRelevantPage(
  milestone: OnboardingMilestone,
  currentPath: string
): boolean {
  return milestone.targetPages.some(
    page =>
      page === currentPath ||
      currentPath.match(new RegExp(page.replace(/\[.*?\]/g, "[^/]+")))
  )
...
export function getCompletedCount(progress: OnboardingProgress): number {
  return ONBOARDING_MILESTONES.filter(
...
export function getCompletionPercentage(progress: OnboardingProgress): number {
  return Math.round(
    (getCompletedCount(progress) / ONBOARDING_MILESTONES.length) * 100
  )
}

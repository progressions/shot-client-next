  ...
  const handleSubmit = async (password: string, passwordConfirmation: string) => {
    ...
    try {
      ...
      if (error_ instanceof Error && "response" in error_) {
        const response = (error_ as { response?: { status?: number; data?: { error?: string } } }).response
        ...
        if (response?.status === 422) {
          ...
          if (data.error?.includes("invalid") || data.error?.includes("expired")) {
            ...
            setError(data.error || "Password reset failed. Please check your password requirements.")
          }
        } else {

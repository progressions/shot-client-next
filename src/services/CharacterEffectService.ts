...
const CharacterEffectService = {
  adjustedMainAttack: function(character: Character, encounter: Encounter): [number, number] {
  ...
  adjustedDamage: function(character: Character, encounter: Encounter): [number, number] {
    return this.adjustedActionValue(character, "Damage", encounter, true)
  },

  adjustedActionValue: function(character: Character, name: string, encounter: Encounter, ignoreImpairments: boolean = false): [number, number] {
  ...
  adjustedValue: function(character: Character, value: number, name: string, encounter: Encounter, ignoreImpairments: boolean = false): [number, number] {
    const effects = FS.characterEffects(encounter, character)
    const matchingEffects = this.effectsForCharacter(character, effects, name)
    const value1 = value
    const value2 = value1 - (ignoreImpairments ? 0 : CS.impairments(character))

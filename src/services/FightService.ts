import type { User, Vehicle, Character, CharacterEffect, Encounter, Shot } from "@/types"
...
const FightService = {
  ...
  users: function(encounter: Encounter): User[] {
    if (!encounter?.id || !encounter?.shots) { return [] }

    const users: User[] = []
    encounter.shots.forEach((shot: Shot) => {
      shot.characters.forEach((char: Character) => {
        if (CS.isPC(char) && char?.user && !users.some(u => u?.id === char?.user?.id)) {
  ...
  playerCharactersForInitiative: function(encounter: Encounter): Character[] {
    return this.playerCharacters(encounter)
      .filter((character) => {
  ...
  characterEffects: function(encounter: Encounter, character: Character): CharacterEffect[] {
    ...
    for (const shot of encounter.shots) {
      const foundChar = shot.characters.find(c => c.shot_id === character.shot_id)
      if (foundChar) {

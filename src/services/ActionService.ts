...
const ActionService = {
  ...
  attacks: function ({
    ...
    for (let i = 0; i < count; i++) {
      attacks.push(
        this.wounds({ swerve, actionValue, defense, stunt, damage, toughness })
      )
    }
    return attacks
  },
  ...
  wounds: function ({
    ...
    const smackdown = this.smackdown({
      swerve,
      actionValue,
      defense,
      stunt,
    ...
    if (verbose) {
      console.log(
        `Wounds ${wounds} : Smackdown ${smackdown.smackdown} - Toughness ${toughness}`
      )
    }
    return {
      ...smackdown,
  ...
  smackdown: function ({
    ...
    const smackdown = outcome.success
      ? (outcome.outcome || 0) + (damage || 0)
      : 0
    if (verbose) {
      console.log(
        `Smackdown ${smackdown} : Outcome ${outcome.outcome} + Damage ${damage || 0}`
      )
    }
    return {
      ...outcome,
  ...
  outcome: function ({
    ...
    if (verbose) {
      console.log(
        `Outcome ${outcome} : ActionResult ${actionResult.actionResult} - Defense ${modifiedDefense}${stunt ? "*" : ""}`
      )
    }
    return {
      ...actionResult,
      ...
      success,
      wayAwfulFailure:
        actionResult.wayAwfulFailure || (actionResult.boxcars && outcome < 0),
    }
  },
  ...
  actionResult: function ({
    ...
    if (verbose) {
      console.log(
        `ActionResult ${result} : Swerve ${rolledSwerve.result} + ActionValue ${actionValue}`
      )
    }
    return {

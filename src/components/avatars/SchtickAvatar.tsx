...
const SchtickAvatar = ({
  ...
  const handleAvatarClick = useCallback(
    (event: React.MouseEvent) => {
      if (!disableImageViewer && schtick?.image_url) {
        event.preventDefault()
        event.stopPropagation()
        setImageViewerOpen(true)
      }
    },
    [schtick?.image_url, disableImageViewer]
  )
  ...
  const hasClickableImage = !disableImageViewer && schtick?.image_url
  ...
  const baseAvatar = (
    <Avatar
      ...
      aria-label={
        hasClickableImage ? `View full image of ${schtick.name}` : schtick.name
      }
    >
      {initials}
    </Avatar>
    ...
    <>
      ...
      {schtick.image_url && (
        <ImageViewerModal
          ...
          imageUrl={schtick.image_url}
          altText={schtick.name}
          entity={schtick}
        />
      )}
    </>

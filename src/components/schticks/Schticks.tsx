...
import { SchtickDetail, CreateSchtickForm, EditSchtickForm } from "@/components/schticks"
import type { Schtick, PaginationMeta } from "@/types/types"
...
export default function Schticks({ initialSchticks, initialMeta, initialSort, initialOrder }: SchticksProps) {
  ...
  const fetchSchticks = useCallback(async (page: number = 1, sort: string = "created_at", order: string = "desc") => {
    try {
      const response = await client.getSchticks({ page, sort, order })
      console.log("Fetched schticks:", response.data.schticks)
      dispatchForm({ type: FormActions.UPDATE, name: "schticks", value: response.data.schticks })
      ...
      dispatchForm({
        ...
        payload: err instanceof Error ? err.message : "Failed to fetch schticks"
      })
      console.error("Fetch schticks error:", err)
    }
  }, [client, dispatchForm])
  ...
  }, [client, campaignData, dispatchForm, fetchSchticks, validSorts, validOrders])
  ...
  const handleSaveSchtick = async (newSchtick: Schtick) => {
    dispatchForm({ type: FormActions.UPDATE, name: "schticks", value: [newSchtick, ...schticks] })
  }

  const handleDeleteSchtick = (schtickId: string) => {
    dispatchForm({ type: FormActions.UPDATE, name: "schticks", value: schticks.filter((schtick) => schtick.id !== schtickId) })
  ...
  const handleSaveEditSchtick = (updatedSchtick: Schtick) => {
    dispatchForm({
      ...
      name: "schticks",
      value: schticks.map((f) => (f.id === updatedSchtick.id ? updatedSchtick : f))
    })
    setSelectedSchtick(null)
  }
  ...
  const handlePageChange = async (_event: React.ChangeEvent<unknown>, page: number) => {
    if (page <= 0 || page > meta.total_pages) {
      router.push(`/schticks?page=1&sort=${sort}&order=${order}`, { scroll: false })
      await fetchSchticks(1, sort, order)
    } else {
      router.push(`/schticks?page=${page}&sort=${sort}&order=${order}`, { scroll: false })
  ...
  const handleSortChange = (event: SelectChangeEvent<string>) => {
    ...
    if (validSorts.includes(newSort)) {
      ...
      router.push(`/schticks?page=1&sort=${newSort}&order=${order}`, { scroll: false })
  ...
  const handleOrderChange = async () => {
    ...
    setOrder(newOrder)
    router.push(`/schticks?page=1&sort=${sort}&order=${newOrder}`, { scroll: false })

...
import { WeaponDetail, CreateWeaponForm, EditWeaponForm } from "@/components/weapons"
import type { Weapon, PaginationMeta } from "@/types/types"
...
export default function Weapons({ initialWeapons, initialMeta, initialSort, initialOrder }: WeaponsProps) {
  ...
  const fetchWeapons = useCallback(async (page: number = 1, sort: string = "created_at", order: string = "desc") => {
    try {
      const response = await client.getWeapons({ page, sort, order })
      console.log("Fetched weapons:", response.data.weapons)
      dispatchForm({ type: FormActions.UPDATE, name: "weapons", value: response.data.weapons })
      ...
      dispatchForm({
        ...
        payload: err instanceof Error ? err.message : "Failed to fetch weapons"
      })
  ...
  const handleSaveWeapon = async (newWeapon: Weapon) => {
    dispatchForm({ type: FormActions.UPDATE, name: "weapons", value: [newWeapon, ...weapons] })
  }

  const handleDeleteWeapon = (weaponId: string) => {
    dispatchForm({ type: FormActions.UPDATE, name: "weapons", value: weapons.filter((weapon) => weapon.id !== weaponId) })
  ...
  const handlePageChange = async (_event: React.ChangeEvent<unknown>, page: number) => {
    if (page <= 0 || page > meta.total_pages) {
      router.push(`/weapons?page=1&sort=${sort}&order=${order}`, { scroll: false })
      await fetchWeapons(1, sort, order)
    } else {
      router.push(`/weapons?page=${page}&sort=${sort}&order=${order}`, { scroll: false })
  ...
  const handleSortChange = (event: SelectChangeEvent<string>) => {
    ...
    if (validSorts.includes(newSort)) {
      ...
      router.push(`/weapons?page=1&sort=${newSort}&order=${order}`, { scroll: false })
  ...
  const handleOrderChange = async () => {
    ...
    setOrder(newOrder)
    router.push(`/weapons?page=1&sort=${sort}&order=${newOrder}`, { scroll: false })

...
import { Autocomplete as MuiAutocomplete, TextField, styled, CircularProgress } from "@mui/material"
...
export function Autocomplete({ label, fetchOptions, onChange, value }: AutocompleteProps) {
  ...
  const noneOption: Option = useMemo(() => ({ label: "None", value: NONE_VALUE }), [])

  const displayOptions = useMemo(() => [noneOption, ...options], [options, noneOption])
  ...
  useEffect(() => {
    ...
    fetchOptions("").then((initialOptions) => {
      dispatchForm({ type: FormActions.UPDATE, name: "options", value: initialOptions })
  ...
  useEffect(() => {
    if (value !== null && !options.some((option) => option.value === value) && !fetchedMissing.current.has(value)) {
      ...
      fetchOptions(value).then((newOptions) => {
        ...
        dispatchForm({ type: FormActions.UPDATE, name: "options", value: updatedOptions })
        dispatchForm({ type: FormActions.EDIT, name: "loading", value: false })
      })
    }
        ...
        if (reason === "input" || reason === "clear") {
          ...
          fetchOptions(newInputValue).then((newOptions) => {
            if (inputRef.current === newInputValue) {
              dispatchForm({ type: FormActions.UPDATE, name: "options", value: newOptions })
              dispatchForm({ type: FormActions.EDIT, name: "loading", value: false })
            }
          })
        }

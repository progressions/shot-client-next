...
import type { EditorChangeEvent, Adventure } from "@/types"
...
type FormStateData = Adventure & {
  [key: string]: unknown
  image?: File | null
  _tempImageFile?: File
}

interface AdventureFormProperties {
  open: boolean
  onClose: () => void
  title: string
}
...
export default function AdventureForm({
  ...
  const { createEntity, handleFormErrors } = useEntity<Adventure>(
    defaultAdventure,
    dispatchForm
  )
  ...
  const handleEntityUpdate = (updatedAdventure: Adventure) => {
    if (updatedAdventure.image_url !== data.image_url) {
      dispatchForm({
        type: FormActions.UPDATE,
        name: "image_url",
        value: updatedAdventure.image_url,
      })
    }

    if (updatedAdventure._tempImageFile !== data._tempImageFile) {
      dispatchForm({
        type: FormActions.UPDATE,
        name: "_tempImageFile",
        value: updatedAdventure._tempImageFile,
      })
    }

    if (updatedAdventure.x_position !== data.x_position) {
      dispatchForm({
        type: FormActions.UPDATE,
        name: "x_position",
        value: updatedAdventure.x_position,
      })
    }

    if (updatedAdventure.y_position !== data.y_position) {
      dispatchForm({
        type: FormActions.UPDATE,
        name: "y_position",
        value: updatedAdventure.y_position,
      })
    }
  }

  const handleNameEntityUpdate = (updatedAdventure: Adventure) => {
    dispatchForm({
      type: FormActions.UPDATE,
      name: "name",
      value: updatedAdventure.name,
    })
    if (errors.name) {
      dispatchForm({
  ...
  const handleNameEntitySave = async (updatedAdventure: Adventure) => {
    dispatchForm({
      type: FormActions.UPDATE,
      name: "name",
      value: updatedAdventure.name,

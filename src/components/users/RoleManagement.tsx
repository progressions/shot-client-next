...
export default function RoleManagement({ user, onUserUpdate }: RoleManagementProps) {
  const { client } = useClient()
  const { toastSuccess, toastError } = useToast()
  const [updating, setUpdating] = useState<string | null>(null)


  const handleRoleToggle = async (role: "admin" | "gamemaster", checked: boolean) => {
    setUpdating(role)
    ...
    try {
      ...
      toastSuccess(`${role === "admin" ? "Admin" : "Gamemaster"} status ${checked ? "granted" : "revoked"} successfully`)
    } catch (error_) {
      console.error(`Failed to update ${role} status:`, error_)
      toastError(`Failed to update ${role === "admin" ? "Admin" : "Gamemaster"} status. Please try again.`)
    } finally {
      setUpdating(null)
    }
  }

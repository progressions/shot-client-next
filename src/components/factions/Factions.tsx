...
import { FactionDetail, CreateFactionForm, EditFactionForm } from "@/components/factions"
import type { Faction, PaginationMeta } from "@/types/types"
...
export default function Factions({ initialFactions, initialMeta, initialSort, initialOrder }: FactionsProps) {
  ...
  const fetchFactions = useCallback(async (page: number = 1, sort: string = "created_at", order: string = "desc") => {
    try {
      ...
      dispatchForm({
        ...
        payload: err instanceof Error ? err.message : "Failed to fetch factions"
      })
  ...
  const handleSaveFaction = async (newFaction: Faction) => {
    dispatchForm({ type: FormActions.UPDATE, name: "factions", value: [newFaction, ...factions] })
  }

  const handleDeleteFaction = (factionId: string) => {
    dispatchForm({ type: FormActions.UPDATE, name: "factions", value: factions.filter((faction) => faction.id !== factionId) })
  ...
  const handleSaveEditFaction = (updatedFaction: Faction) => {
    dispatchForm({
      ...
      name: "factions",
      value: factions.map((f) => (f.id === updatedFaction.id ? updatedFaction : f))
    })
    setSelectedFaction(null)
  }
  ...
  const handlePageChange = async (_event: React.ChangeEvent<unknown>, page: number) => {
    if (page <= 0 || page > meta.total_pages) {
      router.push(`/factions?page=1&sort=${sort}&order=${order}`, { scroll: false })
      await fetchFactions(1, sort, order)
    } else {
      router.push(`/factions?page=${page}&sort=${sort}&order=${order}`, { scroll: false })
  ...
  const handleSortChange = (event: SelectChangeEvent<string>) => {
    ...
    if (validSorts.includes(newSort)) {
      ...
      router.push(`/factions?page=1&sort=${newSort}&order=${order}`, { scroll: false })
  ...
  const handleOrderChange = async () => {
    ...
    setOrder(newOrder)
    router.push(`/factions?page=1&sort=${sort}&order=${newOrder}`, { scroll: false })

...
export default function Sites({
  ...
  const { meta, sites, factions, faction_id, drawerOpen, error } =
    formState.data
    ...
    async (
      ...
      try {
        ...
        dispatchForm({
          ...
          payload: error_ instanceof Error ? error_.message : "Failed to fetch sites",
        })
        console.error("Fetch sites error:", error_)
      }
    },
    [client, dispatchForm]
  )
  ...
  const handleDeleteSite = (siteId: string) => {
    dispatchForm({
      type: FormActions.UPDATE,
      name: "sites",
      value: sites.filter(site => site.id !== siteId),
    })
  ...
  const handleSortChange = (event: SelectChangeEvent<string>) => {
    ...
    if (validSorts.includes(newSort)) {
      ...
      router.push(`/sites?page=1&sort=${newSort}&order=${order}`, {
        scroll: false,
      })
      ...
      <Stack spacing={2} sx={{ mb: 4 }}>
        ...
        <Box sx={{ pb: 2 }}>
          <Typography>
            ...
            whose flow of energy produces powerful <InfoLink info="Chi" /> for
            those who are attuned to it. A Feng Shui Site belongs to a{" "}
            <InfoLink href="/factions" info="Faction" />.
          </Typography>
        </Box>
        <Box
          sx={{

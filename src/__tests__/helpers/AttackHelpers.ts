...
export function expectAttack(attacker: Person, target: Person, dieRoll: number, stunt: boolean = false) {
...
export function expectAttackerUnharmed(initialAttacker: Person, finalAttacker: Person) {
  // Verify attacker wasn't harmed
  expect(finalAttacker.action_values.Wounds).toEqual(initialAttacker.action_values.Wounds)
}

export function expectTargetUnharmed(initialTarget: Person, finalTarget: Person) {
  // Verify target wasn't harmed  
  expect(finalTarget.action_values.Wounds).toEqual(initialTarget.action_values.Wounds)
}